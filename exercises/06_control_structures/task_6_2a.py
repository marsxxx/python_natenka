...
"""
work = True #переменная означающая, что скрипт можно выполнять дальше
ip = input("Введите ip-адрес в формате Х.Х.Х.Х: ")
octets = ip.split(".")  #разделили ip на октеты

if len(octets)<4 or len(octets)>4:
    print ("Неправильный IP-адрес")
    work = False #если октетов не 4, то заканчиваем выполнение скрипта
else:
    try:
        for i in range(4):
        ...
        print("Неправильный IP-адрес")
        work = False #если во время преобразования октета в число есть ошибка, то заканчиваем выполнение скрипта
